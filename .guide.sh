...
center "~ Dan-Guide ~"
echo "Вы запустили неофициальный гид по установке ArchLinux. Для переключения между панелями используйте любую из этих клавиш:
ctrl+j
ctrl+n
ctrl+f
ctrl+t
Для продолжения нажмите Enter, когда закроете верхнюю подсказку."
